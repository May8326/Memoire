...
\begin{itemize}
	...
	\item et ceux de l'\gls{emediatheque}, gérés dans le logiciel \textit{Skopus} dédié aux documents iconographiques et audiovisuels.
\end{itemize}
Ces trois corpus de termes, bien qu'ils partagent une ambition commune – ordonner, nommer, rendre trouvable – ne sont pas nés du même mouvement ni selon les mêmes logiques. Chacun de ces trois ensembles est en réalité constitué de plusieurs \gls{thesaurus} ou listes d'\gls{autorite}\footnote{Voir Glossaire \gls{autorite}} distinctes, dont les plus importants sont ceux des mots-clés et des constructeurs d'aéronefs, qui se retrouvent dans les champs d'indexation des collections.
...
Sur le plan technique, les divergences entre logiciels rendent toute interopérabilité complexe. Chaque base repose sur une architecture distincte\footnote{Un schéma réalisé dans le cadre du stage représente les divers flux de données -- dont on voit donc le cloisonnement -- associés à ces applications en Annexe \refinterne{Ax-C}}~:
\begin{itemize}
	...
	\item Le musée, de son côté, utilisait \gls{micromusee} (v6) depuis 2000, version qui a peu évolué depuis et dont les difficultés d'utilisation et l'ergonomie limitée ont certainement freiné la réflexion sur le \gls{thesaurus} qui y était intégré. Ce logiciel a également été remplacé en juillet 2025 par \gls{archange}, une déclinaison du logiciel \textit{S-Museum} développée pour les établissements du \minarm.
